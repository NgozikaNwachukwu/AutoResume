...
PASSIVE_TO_ACTIVE = [
    (
        r"\bwas\b\s+(tested|built|created|developed|implemented|designed|managed)\b",
        r"\1",
    ),
    (
        r"\bwere\b\s+(tested|built|created|developed|implemented|designed|managed)\b",
        r"\1",
    ),
]
...
def make_star_bullets(
    summary: str, role: str = "", org: str = "", tools=None
) -> list[str]:
    from nltk.tokenize import sent_tokenize

    sents = [
        _clean_sentence(s) for s in sent_tokenize(summary or "") if _clean_sentence(s)
    ]
    joined = " ".join(sents)
    ...
    b3 = "• " + _normalize_tech_in_text(
        ("Delivered measurable outcomes by " + res).rstrip(".") + "."
    )
    return [b1, b2, b3]
...
def make_xyz_bullets(summary: str, tools=None, max_bullets: int = 2) -> list[str]:
    ...
    how = (
        _strong_opening(sents[1])
        if len(sents) > 1
        else "implementing core features and tests"
    )
    ...
    for d in sents[2:][: max(0, max_bullets - 1)]:
        bullets.append(
            _normalize_tech_in_text("• " + _strong_opening(d).rstrip(".") + ".")
        )
...
def build_resume(raw: dict) -> dict:
    ...
    for proj in raw.get("projects", []):
        ...
        bullets = (
            make_xyz_bullets(source_text, tools=proj.get("tools"))
            if source_text
            else []
        )
